    ...
    const getProduct = async () => {
      try {
        ...
        const response = await fetch(
          `https://product-data-2pdk.onrender.com/products/${id}`
        );
        if (!response.ok) {
        ...
        // Log category ID to check if it's correctly retrieved
        console.log("Category ID:", data.category?.id);

        // Fetch similar products if category exists
        if (data.category?.id) {
          const response2 = await fetch(
            `https://product-data-2pdk.onrender.com/categories/${data.category.id}/products`
          );
          if (!response2.ok) {
